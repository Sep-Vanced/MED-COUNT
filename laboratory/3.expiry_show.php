...
$sql = "SELECT * FROM laboratory WHERE `exp.date` <= DATE_ADD(CURDATE(), INTERVAL 3 MONTH) AND `exp.date` >= CURDATE() AND status != 'Deleted' ORDER BY `exp.date` ASC";
...
if ($result->num_rows > 0) {
    ...
    echo "<tr><td colspan='10'>No near expiry items found</td></tr>";
}
?>