                                ...
                                <div class="text-center mb-4">
                                    ...
                                    include('../config/config.php');
                                        ...
                                        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                                            ...
                                            if ($product_id !== null && $transfer_quantity !== null) {
                                                ...
                                                try {
                                                    // Step 1: Fetch product details from the laboratory
                                                    $select_sql = "SELECT * FROM laboratory WHERE id = ?";
                                                    ...
                                                    // Step 2: Calculate the new 'onhand' quantity for the laboratory after the usage
                                                    $new_onhand = $product_details['onhand'] - $transfer_quantity;

                                                    if ($new_onhand < 0) {
                                                        throw new Exception("Insufficient stock in the laboratory to mark as used.");
                                                    }

                                                    // Step 3: Update the 'onhand' quantity in the laboratory
                                                    $update_laboratory_sql = "UPDATE laboratory SET onhand = ? WHERE id = ?";
                                                    $stmt = mysqli_prepare($conn, $update_laboratory_sql);
                                                    mysqli_stmt_bind_param($stmt, 'ii', $new_onhand, $product_id);
                                                    mysqli_stmt_execute($stmt);

                                                    if (mysqli_stmt_affected_rows($stmt) < 1) {
                                                        throw new Exception("Failed to update laboratory inventory.");
                                                    }
                                                    ...
                                                    $user = 'laboratory'; // Define a variable for the user
                                                    ...
                                                    mysqli_commit($conn);
                                                    echo "Product usage successfully recorded in the laboratory!";
                                                } catch (Exception $e) {
                                            ...
                                            if (isset($_GET['id'])) {
                                                $product_id = $_GET['id'];
                                                $sql = "SELECT * FROM laboratory WHERE id = ?";
                                    ...
                                    <form class="justify-content-start align-items-center" action="" method="post" enctype="multipart/form-data" style="padding-left: 20px; padding-right: 20px;">
                                            ...
                                            <div class="column">
                                                ...
                                                <div class="col mb-3">
                                                    <label class="form-label">Stock Bal (Laboratory)</label>
