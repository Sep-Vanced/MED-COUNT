...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['id'])) {
    ...
    try {
        ...
        $delivery_sql = "SELECT d.*, so.image_filename FROM deliveries d 
                         JOIN supply_office so ON d.product_name = so.product_name
                         WHERE d.id = ?";
        ...
        if (mysqli_stmt_affected_rows($stmt) < 1) {
            ...
            $image_filename = $delivery_details['image_filename'] ?? ''; // Include the image filename

                // Step 2: Add the product including the image filename to the pharmacy's inventory
                $insert_pharmacy_sql = "INSERT INTO pharmacy (product_name, product_num, unit_of_issue, batch_num, `exp.date`, unit_value, onhand, stock_bal, image_filename) 
                                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
                $stmt = mysqli_prepare($conn, $insert_pharmacy_sql);
                ...
                $image_filename  // Include the image filename
            );
            mysqli_stmt_execute($stmt);
        ...
        // Step 3: Add an entry to the used_items table
        $used_date = date('Y-m-d H:i:s'); // Current timestamp
        $user = 'Supply Office'; // Issued by Supply Office
        $insert_used_sql = "INSERT INTO used_items (product_name, product_num, unit_of_issue, batch_num, `exp.date`, unit_value, used_quantity, used_date, user, issued) 
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        mysqli_stmt_bind_param($stmt, 'sssssissss', 
            ...
            $used_date, 
            $user, 
            $user  // For 'issued', use "Supply Office"
        );
        mysqli_stmt_execute($stmt);

        if (mysqli_stmt_affected_rows($stmt) < 1) {
            throw new Exception("Failed to add item to the used_items table.");
        }
