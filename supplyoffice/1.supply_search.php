...
$sql = "SELECT * FROM supply_office WHERE status != 'Deleted' OR status IS NULL AND (image_filename LIKE ? OR product_name LIKE ? OR product_num LIKE ? OR unit_of_issue LIKE ? OR batch_num LIKE ? OR `exp.date` LIKE ? OR unit_value LIKE ? OR onhand LIKE ? OR stock_bal LIKE ?) ORDER BY product_name ASC";
$search = isset($_GET['search']) ? $_GET['search'] : '';    
$stmt = $conn->prepare($sql);
$searchParam = "%{$search}%";
$stmt->bind_param("sssssssss", $searchParam, $searchParam, $searchParam, $searchParam, $searchParam, $searchParam, $searchParam, $searchParam, $searchParam);
$stmt->execute();
