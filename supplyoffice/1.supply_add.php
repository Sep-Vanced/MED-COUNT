                                            ...
                                            if (isset($_FILES['product_image']) && $_FILES['product_image']['error'] == 0) {
                                                ...
                                                if (in_array(strtolower($file_ext), $allowed_ext)) {
                                                    ...
                                                    if (move_uploaded_file($_FILES['product_image']['tmp_name'], $upload_path)) {
                                                        ...
                                                        $insert_query = "INSERT INTO supply_office (product_name, product_num, unit_of_issue, batch_num, `exp.date`, unit_value, onhand, stock_bal, image_filename) 
                                                        VALUES ('$product_name', '$product_num', '$unit_of_issue', '$batch_num', '$exp_date', '$unit_value', '$onhand', '$stock_bal', '$new_filename')";
                                    
                                                        if (mysqli_query($conn, $insert_query)) {
                                                            echo '<div class="alert alert-success" role="alert">Record added successfully.</div>';
                                                        } else {
                                                            echo '<div class="alert alert-danger" role="alert">Error adding record. Please try again.</div>';
                                                        }
                                                    } else {
                                                        echo '<div class="alert alert-danger" role="alert">Error uploading file. Please try again.</div>';
                                                    }
                                                } else {
                                                    echo '<div class="alert alert-danger" role="alert">Invalid file type. Only JPG, JPEG, PNG, and GIF are allowed.</div>';
                                                }
                                            } else {
                                                echo '<div class="alert alert-danger" role="alert">Please upload a product image.</div>';
                                            }
                                        }
                                    }
                                    ?>
