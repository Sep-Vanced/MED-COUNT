...
$sql = "SELECT * FROM central_supply_office WHERE status != 'Deleted' OR status IS NULL AND (product_name LIKE ? OR product_num LIKE ? OR unit_of_issue LIKE ? OR batch_num LIKE ? OR `exp.date` LIKE ? OR unit_value LIKE ? OR onhand LIKE ? OR stock_bal LIKE ?) ORDER BY product_name ASC";
