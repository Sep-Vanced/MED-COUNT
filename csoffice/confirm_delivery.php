...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['id'])) {
    ...
    try {
        ...
        if (mysqli_stmt_affected_rows($stmt) < 1) {
            ...
            $insert_central_supply_office_sql = "INSERT INTO central_supply_office (product_name, product_num, unit_of_issue, batch_num, `exp.date`, unit_value, onhand, stock_bal) 
                                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = mysqli_prepare($conn, $insert_central_supply_office_sql);
        ...
        if (mysqli_stmt_affected_rows($stmt) < 1) {
            throw new Exception("Failed to add product to the central supply office inventory.");
        }
